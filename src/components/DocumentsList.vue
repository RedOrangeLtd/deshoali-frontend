              ...
              <div v-if="true">
                <div class="title-wrapper">
                  <div class="title-icon">
                    <v-icon color="white" size="large">mdi-file-document-multiple-outline</v-icon>
                  </div>
                  <h2 class="section-title mb-2">Resources</h2>
                </div>
              </div>
            </transition>
          </v-col>
        ...
        <v-row>
          <template v-for="item in filteredItems" :key="item.id">
            <v-col cols="12" class="pl-0 pr-0 pt-1 pb-3">
              <div class="video-card mx-auto" @click="viewDocument(item)">
                <div class="card-content">
                  <div class="video-icon-container">
                    <v-icon class="video-icon" icon="mdi-file"></v-icon>
                  ...
                  <div class="video-info">
                    ...
                    <div class="video-meta">
                      <div class="description-wrapper">
                        <!-- Down arrow when collapsed and overflowing -->
                        <v-btn
                          v-if="isOverflowing(item.id) && !isExpanded(item.id)"
                        ...
                        <!-- Up arrow pinned to top-right when expanded -->
                        <v-btn
                          v-if="isExpanded(item.id)"
                          class="arrow-btn top-collapse-btn"
                          density="compact"
                          variant="text"
                          icon
                          @click="toggleExpand(item.id)"
                          ...
                          size="small"
                        >
                          <v-icon size="18" icon="mdi-chevron-up" />
                        </v-btn>

                        <div
                          class="desc-text"
                          :class="{ 'clamped': !isExpanded(item.id), 'expanded': isExpanded(item.id) }"
                          :ref="el => setDescRef(item.id, el)"
                        >
                          {{ item.description }}
                        </div>
                      </div>
...
export default {
  ...
  data() {
    return {
      ...
      showPlayer: false,
      selectedItem: null,
      // state for subtitle expand/collapse and overflow detection
      expandedDesc: {},
      overflowDesc: {},
      descRefs: {},
    }
  ...
  methods: {
    loadData() {
      const url = apiUrl + '/documents/list'
      this.$axios.get(url)
        .then(response => {
    ...
    downloadData(data) {
      ...
      const fileUrl = `${apiUrl}/documents/download/${data.id}?t=${timestamps}`;
    ...
    viewDocument(item) {
      const fileUrl = `${baseApiUrl}${item.filepath}`;

      // Check if file is DOCX based on name or add a file_type property to your item
      const isDocx = item.filepath.toLowerCase().endsWith('.docx') ||
      ...
      if (isDocx) {
        ...
        // Open PDF and other files directly
    ...
    setDescRef(id, el) {
      if (el) {
        this.descRefs[id] = el
        // measure when ref is set to know if it overflows
        this.$nextTick(() => this.measureOverflowFor(id))
      }
    },
    isExpanded(id) {
    ...
    toggleExpand(id) {
      const next = !this.expandedDesc[id]
      this.expandedDesc[id] = next
      this.$nextTick(() => this.measureOverflowFor(id))
    },
    measureOverflow() {
      // recompute for all known refs
      Object.keys(this.descRefs).forEach(id => this.measureOverflowFor(id))
    },
    measureOverflowFor(id) {
      const el = this.descRefs[id]
      if (!el) return
      // temporarily clamp to single line for measuring initial overflow when collapsed
      const isExpanded = !!this.expandedDesc[id]
      if (isExpanded) {
        // If expanded, overflow check should be based on collapsed state to decide button visibility
      ...
      this.overflowDesc[id] = overflowing
    },
  }
}
</script>
...
/* Main section with background (borrowed from VideosList for consistent design) */
.video-section {
  background: #ffffff;
  overflow: hidden;
  position: relative;
...
/* Card styles (kept class names for consistency) */
.video-card {
...
.desc-text.expanded {
  ...
  word-wrap: break-word;
  overflow-wrap: break-word;
  padding-right: 36px; /* room for top-right arrow */
}
...
@media (max-width: 768px) {
  ...
  .title-icon :deep(.v-icon) {
    font-size: 20px !important; /* slightly smaller icon on mobile */
  }
...
/* Keep v-card title override removed as we no longer use v-card titles here */
